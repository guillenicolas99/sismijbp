<x-app-layout>
    ...
    <div class="flex flex-col gap-3">
        <p
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 dark:bg-gray-700 dark:text-white w-1/4">
            {{ $user->name }}
        </p>
    ...
    <form action="{{ route('users.update', $user->id) }}" method="POST" class="my-3">
        ...
        <h4 class="text-2xl text-white mb-3">Roles</h4>
        <div class="flex flex-wrap gap-4">
            @foreach ($roles as $rol)
                <label class="flex items-center gap-2 text-gray-700">
                    <input type="checkbox" value="{{ $rol->name }}" name="roles[]" class="mr-2"
