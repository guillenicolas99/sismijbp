<x-app-layout>
    <x-title-form>Listado de todos los {{ $titulo->nombre }}</x-title-form>
    <x-back-button />

    @if (count($personas) >= 1)
        <x-table :columns="['#', 'Nombre', 'Genero', 'Cargo', 'Teléfono', 'Red', 'Estado', 'Acciones']">
    ...
    @else
        <x-alert-blue>NO HAY {{ $titulo->nombre }} registrado</x-alert-blue>
    @endif
</x-app-layout>
